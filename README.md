# instant-blue.pw-docker-provision
instant-blue.pwをいい感じに構築/運用するためのレポジトリ  
...
| パス | 内容 |
|---|---|
| `compose.yaml` | proxy(Caddy), maintenance, app(misskey-tempura), redis, db(PGroonga), backup(misskey-backup) |
...
データは名前付きvolumeに保存される(`instant-bluepw-docker-provision_` + `pg-18`, `redis`, `caddy-data`, `caddy-config`, `caddy-logs`, `mi-backups`)。
