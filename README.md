...
Clone the instant-blue.pw-docker-provision repository from GitHub to server:  
```bash
git clone --recursive https://github.com/miel-misskey/instant-blue.pw-docker-provision.git
```
...
```
cd ./instant-blue.pw-docker-provision
...
> [!NOTE]
> Although the `--profile` option is included only for the first boot, Caddy Proxy does not need to be recreated for updates or other tasks, so it can be run without the `--profile` option with `sudo docker compose up -d --build`.
...
Since ssh connections are made via tailscale, it is recommended to restrict inbound communication and only allow specific traffic.  
...
If you need to update Caddy Proxy, please add the `--profile proxy` option as needed.  
