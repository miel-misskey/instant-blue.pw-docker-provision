...
OLD_SSH="${OLD_SSH:-ssh -i $HOME/.ssh/miel-servers -o StrictHostKeyChecking=accept-new root@133.130.91.223}"
NEW_SSH="${NEW_SSH:-ssh -i /Users/mq1/miel-misskey/miel-kagoya-misskey.key -o IdentitiesOnly=yes -o PubkeyAcceptedAlgorithms=+ssh-rsa -o StrictHostKeyChecking=accept-new root@133.18.114.59}"
REMOTE_DIR="${REMOTE_DIR:-/opt/instant-blue.pw-docker-provision}"
...
R2_PREFIX="${R2_PREFIX:-}"
BASE_URL="${BASE_URL:?set BASE_URL, e.g. https://media.instant-blue.pw}"
NEW_DB="${NEW_DB:-mk1}"
...
$OLD_SSH "rsync -a --info=progress2 '$OLD_FILES' -e 'ssh -o StrictHostKeyChecking=accept-new' \
  root@133.18.114.59:'$REMOTE_DIR/data/files/'" || \
